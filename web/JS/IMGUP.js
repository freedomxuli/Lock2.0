...
var IMG_LENGTH = 20;//图片最大1MB
var IMG_MAXCOUNT = 1;//最多选中图片张数
...
$("#div_file1").click(function () {
    $("#sm1").empty();
    if ($(".lookimg1").length >= IMG_MAXCOUNT) {
        alert("一次最多上传" + IMG_MAXCOUNT + "张图片");
        return;
    }

    var _CRE_FILE = document.createElement("input");
    if ($(".imgfile1").length <= $(".lookimg1").length) {//个数不足则新创建对象
        _CRE_FILE.setAttribute("type", "file");
        _CRE_FILE.setAttribute("class", "imgfile1");
        _CRE_FILE.setAttribute("accept", ".png,.jpg,.jpeg");
        _CRE_FILE.setAttribute("num", UP_IMGCOUNT);//记录此对象对应的编号
        $("#div_file1").after(_CRE_FILE);
    }
    else { //否则获取最后未使用对象
        _CRE_FILE = $(".imgfile1").eq(0).get(0);
    }
    return $(_CRE_FILE).click();//打开对象选择框
});
...
$(".imgfile1").live("change", function () {
    if ($(this).val().length > 0) {//判断是否有选中图片
        ...
        //创建预览外层
        var _prevdiv = document.createElement("div");
        _prevdiv.setAttribute("class", "lookimg1");
        //创建内层img对象
        var preview = document.createElement("img");
        $(_prevdiv).append(preview);
        //创建删除按钮
        var IMG_DELBTN = document.createElement("div");
        IMG_DELBTN.setAttribute("class", "lookimg_delBtn1");
        IMG_DELBTN.innerHTML = "移除";
        $(_prevdiv).append(IMG_DELBTN);
        //创建进度条
        var IMG_PROGRESS = document.createElement("div");
        ...
        _prevdiv.setAttribute("num", $(this).attr("num"));
        //对象注入界面
        $("#div_imglook1").children("div:last").before(_prevdiv);
        UP_IMGCOUNT++;//编号增长防重复
...
//删除选中图片
$(".lookimg_delBtn1").live("click", function () {
    $(".imgfile1[num=" + $(this).parent().attr("num") + "]").remove();//移除图片file
    $(this).parent().remove();//移除图片显示
    $("#sm1").html("手持身份证照片反面上传");
});

//删除按钮移入移出效果
$(".lookimg1").live("mouseover", function () {
    if ($(this).attr("ISUP") != "1")
        $(this).children(".lookimg_delBtn1").eq(0).css("display", "block");
});
$(".lookimg1").live("mouseout", function () {
    $(this).children(".lookimg_delBtn1").eq(0).css("display", "none");
});


//********************************************************************//

$("#div_file2").click(function () {
    $("#sm2").empty();
    if ($(".lookimg2").length >= IMG_MAXCOUNT) {
        alert("一次最多上传" + IMG_MAXCOUNT + "张图片");
        return;
    }

    var _CRE_FILE = document.createElement("input");
    if ($(".imgfile2").length <= $(".lookimg2").length) {//个数不足则新创建对象
        _CRE_FILE.setAttribute("type", "file");
        _CRE_FILE.setAttribute("class", "imgfile2");
        _CRE_FILE.setAttribute("accept", ".png,.jpg,.jpeg");
        _CRE_FILE.setAttribute("num", UP_IMGCOUNT);//记录此对象对应的编号
        $("#div_file2").after(_CRE_FILE);
    }
    else { //否则获取最后未使用对象
        _CRE_FILE = $(".imgfile2").eq(0).get(0);
    }
    return $(_CRE_FILE).click();//打开对象选择框
});
...
$(".imgfile2").live("change", function () {
    if ($(this).val().length > 0) {//判断是否有选中图片
        ...
        //创建预览外层
        var _prevdiv = document.createElement("div");
        _prevdiv.setAttribute("class", "lookimg2");
        //创建内层img对象
        var preview = document.createElement("img");
        $(_prevdiv).append(preview);
        //创建删除按钮
        var IMG_DELBTN = document.createElement("div");
        IMG_DELBTN.setAttribute("class", "lookimg_delBtn2");
        IMG_DELBTN.innerHTML = "移除";
        $(_prevdiv).append(IMG_DELBTN);
        //创建进度条
        var IMG_PROGRESS = document.createElement("div");
        ...
        _prevdiv.setAttribute("num", $(this).attr("num"));
        //对象注入界面
        $("#div_imglook2").children("div:last").before(_prevdiv);
        UP_IMGCOUNT++;//编号增长防重复
...
//删除选中图片
$(".lookimg_delBtn2").live("click", function () {
    $(".imgfile2[num=" + $(this).parent().attr("num") + "]").remove();//移除图片file
    $(this).parent().remove();//移除图片显示
    $("#sm2").html("健康证上传（非必填）");
});

//删除按钮移入移出效果
$(".lookimg2").live("mouseover", function () {
    if ($(this).attr("ISUP") != "1")
        $(this).children(".lookimg_delBtn2").eq(0).css("display", "block");
});
$(".lookimg2").live("mouseout", function () {
    $(this).children(".lookimg_delBtn2").eq(0).css("display", "none");
});